...
export default function Layout({ title, children }: LayoutProps) {
  const { user: userOn, logout } = useContext(AuthContext);

  const user = {
    name: userOn.displayName,
    email: userOn.email,
    imageUrl: profile,
  };
                            ...
                            {userNavigation.map((item) => (
                              <Menu.Item key={item.name}>
                                {({ active }) => (
                                  <a
                                    onClick={
                                      item.name === "Sair" ? handleLogout : null
                                    }
                                    className={classNames(
                                      active ? "bg-gray-100" : "",
                  ...
                  <div className="mt-3 px-2 space-y-1">
                    {userNavigation.map((item) => (
                      <Disclosure.Button
                        ...
                        className="block w-full px-3 py-2 rounded-md text-left text-base font-medium text-gray-400 hover:text-white hover:bg-gray-700"
                      >
                        {item.name}
                      </Disclosure.Button>
                    ))}
